...
- **端口被占用**：`start.sh`/`start.bat` 会自动清理"确认是 webcli 自己"的残留进程再启动；如果占用的是别的程序（或者是一个卡死、命令行特征对不上的旧 webcli 进程，脚本不敢确认就不会杀），会提示你换端口，不会误杀。Windows 上现在会顺带打印占用者的 PID（`netstat -ano` 那一行），确认不是别的正经程序的话可以自己 `taskkill /PID <PID> /F`；macOS/Linux 同理看 `lsof` 那几行
