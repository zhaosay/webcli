...
function attachSocket(entry, ws, meta) {
  ...
  const scheduleKill = () => {
    // This ws may already have been superseded by a newer reconnect before
    // its own close/error event got around to firing — don't let a stale
    // handler tear down the session that's actually still connected.
...
function createSession(sid) {
  ...
  const entry = {
    sid, pty: ptyProcess, ws: null, killTimer: null, device: null, ip: null, connectedAt: Date.now(), logStream,
  };
