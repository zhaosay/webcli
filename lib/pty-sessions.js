...
function attachSocket(entry, ws, meta, isNewSession) {
  ...
  ws.on('message', (raw) => {
    ...
    if (msg.type === 'input' && typeof msg.data === 'string') {
      ...
      // Same nudge as on (re)connect: a plain resize() doesn't reliably make
      // a full-screen/raw-mode program (vim, an Ink-based CLI, etc.) redraw
      // at the new width — it can keep painting at the stale column count it
      // last measured, clipping instead of wrapping. Forcing a real size
      // delta (down a row, then back) makes it re-measure for real.
      syncSize(entry, { forceRepaint: true });
  ...
  const detach = () => {
    ...
    entry.killTimer = setTimeout(() => {
      if (sessions.get(entry.sid) === entry) sessions.delete(entry.sid);
