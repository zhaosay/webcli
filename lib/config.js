...
// PROJECT_PORT is a one-off override (e.g. `PROJECT_PORT=3060 ./restart.sh --bg`)
// and always wins; port.txt is the persisted setting written by `./port.sh set`.
function resolvePort() {
  if (process.env.PROJECT_PORT) return Number(process.env.PROJECT_PORT);
  try {
    const saved = fs.readFileSync(PORT_FILE, 'utf8').trim();
