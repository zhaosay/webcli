...
const {
  MAX_NAME_LENGTH, UPLOAD_DIR, MAX_UPLOAD_BYTES, UPLOAD_RETENTION_DAYS,
  USE_TLS, PORT, DATA_DIR, QUICK_COMMANDS_FILE,
} = require('./config');
const {
...
function buildAssets() {
  ...
  registerAsset('/app.js', fs.readFileSync(path.join(__dirname, '..', 'public', 'app.js')), 'application/javascript; charset=utf-8');
  registerAsset('/theme-init.js', fs.readFileSync(path.join(__dirname, '..', 'public', 'theme-init.js')), 'application/javascript; charset=utf-8');
  const vendor = {
...
// Uploads have no built-in owner (unlike session logs, which are always
// created by this process) so cleanup can't distinguish "the user still
// wants this file" from "forgot to move it" — mtime-based retention, same
// policy as session-log.js's cleanupOldLogs, is the best available proxy.
...
function saveQuickCommands(list) {
  ...
  fs.writeFileSync(QUICK_COMMANDS_FILE, JSON.stringify(list), { mode: 0o600 });
}
...
function createRequestHandler() {
  ...
  return (req, res) => {
    ...
    return dispatch(req, res);
      ...
      console.error('[webcli] request handler error:', err);
...
module.exports = { createRequestHandler, cleanupOldUploads };
